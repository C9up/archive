...
describe("archive > configure", () => {
	it("registers the provider and writes the config it reads", async () => {
		...
		expect(providers).toEqual(["@c9up/archive/provider"]);
		expect(files.map((f) => f.path)).toEqual(["config/archive.ts"]);
	});
	...
	it("declares the environment variables the config reads", async () => {
		...
		expect(env).toHaveProperty("DRIVE_DISK");
	...
	it("writes a config that imports from the package it configures", async () => {
		...
		expect(files[0]?.content).toContain("from '@c9up/archive'");
	});
});
